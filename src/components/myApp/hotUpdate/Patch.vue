  ...
  <div class="container">
    ...
    <div class="grid-package">
      ...
      <div class="package-list">
        ...
        <template v-for="(element, index) in packageListArray">
          <div class="package-item">
            ...
            <draggable :list="element.version" class="drag-area-package" :options="{group:{name:'people', pull:false}}" @change="listChange($event, index)">
    ...
    <div class="grid-version">
      ...
      <div class="version-list">
        ...
        <draggable :list="versionListArray" class="drag-area-version" :options="{group:{ name:'people',  pull:'clone', put:true }}">
          <div class="drag-item" v-for="(item, index) in versionListArray">
            <div class="item-row item-row-border">
              ...
              <el-dropdown>
                ...
                <el-dropdown-menu slot="dropdown">
                  <div @click="editVersionHandle(item, index)">
                    <el-dropdown-item>编辑</el-dropdown-item>
                  </div>
  ...
  export default {
    ...
    data() {
      return {
        ...
        currentEditVersionIndex: '',  // 当前编辑的补丁的下标
    ...
    computed: {
      ...
      ...mapGetters({
        ...
        createVersionSuccess: 'createVersionSuccess', // 创建version是否成功
        createVersionData: 'createVersionData', // 创建后的version数据
        editVersionSuccess: 'editVersionSuccess', // 编辑version是否成功
        editVersionData: 'editVersionData', // 编辑后的version数据
        delVersionSuccess: 'delVersionSuccess', // 删除version是否成功
      }),
    },
    watch: {
      packageListShouldReload: function xx() {  // package列表是否需要重新加载
        if (this.packageListShouldReload) {  // 需要重新加载
          ...
          this.getPackageList(1);  // 重新获取应用的package列表
        }
      },
      ...
      createVersionSuccess: function xx() { // 创建补丁成功
        if (this.createVersionSuccess) {
          this.addVersionFormVisible = false; // 关闭创建补丁弹出框
          this.versionListArray.unshift(this.createVersionData); // 在补丁数组中新增创建的补丁项
        }
      },
      editVersionSuccess: function xx() { // 更新补丁成功
        if (this.editVersionSuccess) {
          console.log(this.currentEditVersionIndex);
          // 如果编辑version成功，就从补丁列表数组中更新这一项
          // this.versionListArray.splice(this.currentDelVersionIndex, 1);
          this.addVersionFormVisible = false; // 关闭编辑补丁弹出框
          this.versionListArray[this.currentEditVersionIndex] = this.editVersionData; // 更新补丁数组中编辑的补丁项
          // 因为编辑的补丁可能已经存在于某一个package中，为了数据同步，首先清空package列表，然后重新获取package列表第一页的数据
          this.packageListArray = [];
          this.getPackageList(1);
        }
      },
      delVersionSuccess: function xx() {  // 删除补丁成功
        if (this.delVersionSuccess) {
          console.log(this.currentDelVersionIndex);
          // 如果删除version成功，就从补丁列表数组中删除这一项
          this.versionListArray.splice(this.currentDelVersionIndex, 1);
          // 因为删除的补丁可能已经存在于某一个package中，为了数据同步，首先清空package列表，然后重新获取package列表第一页的数据
          this.packageListArray = [];
          this.getPackageList(1);
        }
      },
    ...
    created: function created() {
      this.getPackageList(1);  // 获取应用的package列表第一页数据
      this.getVersionList(1);  // 获取应用的补丁列表第一页数据
    },
    methods: {
      getPackageList(currentPage) {  // 获取package列表数据
        this.packageListLoadMoreLoading = true;  // loading中
        packageApi.list({ app_key: this.app_key, page: currentPage }).then((response) => {
          this.packageListLoadMoreLoading = false;  // loading结束
          if (response.errno === 0) { // 获取package列表成功
            response.data.data.forEach((item) => {  // 保存package列表数据
              this.packageListArray.push(item);
            });
            // this.packageListArray = response.data.data;  // 保存package列表数据
            // console.log(this.packageListArray);
            this.packageListTotalPages = response.data.totalPages; // 保存分页信息-总页数
          } else {  // 获取package列表失败
      ...
      packageListLoadMore() { // package列表加载更多
        this.packageListCurrentPage += 1;
        this.getPackageList(this.packageListCurrentPage);
      },
      getVersionList(currentPage) {  // 获取补丁列表数据
        this.versionListLoadMoreLoading = true;  // loading中
        versionApi.list({ app_key: this.app_key, page: currentPage }).then((response) => {
          this.versionListLoadMoreLoading = false;  // loading结束
          if (response.errno === 0) { // 获取补丁列表成功
            response.data.data.forEach((item) => {  // 保存补丁列表数据
              this.versionListArray.push(item);
            });
            // this.versionListArray.concat(response.data.data);  // 保存补丁列表数据
            // console.log(this.versionListArray);
            this.versionListTotalPages = response.data.totalPages; // 保存分页信息-总页数
          } else {  // 获取补丁列表失败
            this.$message.error(response.errmsg);
          }
        })
      ...
      versionListLoadMore() { // 补丁列表加载更多
        this.versionListCurrentPage += 1;
        this.getVersionList(this.versionListCurrentPage);
      },
      listChange(evt, index) {  // package列表中的补丁发生改变时的事件
        // console.log(this.packageListArray[index]);
      ...
      editVersionHandle(versionInfo, index) { // 补丁列表项的dropdown编辑按钮事件处理
        // console.log(versionInfo);
        this.currentEditVersionKey = versionInfo.key; // 记录当前编辑的补丁的key
        this.currentEditVersionIndex = index; // 记录当前编辑的补丁的index
        this.qiniuResponse = {};  // 清空上传文件成功后七牛返回的hash和name等信息
        ...
        this.addVersionForm.grayType = versionInfo.gray_type || 1;  // 灰度下发类型
        this.addVersionForm.grayPercent = versionInfo.gray_percent * 10 || 20;  // 灰度百分比
        this.addVersionForm.grayCount = versionInfo.gray_count || 100;  // 灰度设备数量
